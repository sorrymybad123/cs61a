...
def balanced(m):
    ...
    left_torque, right_torque = length(left_arm) * total_weight(end(left_arm)), length(right_arm) * total_weight(end(right_arm))
...
def has_path(t, word):
    ...
    def helper(t, cur):
        ...
        if cur not in word: 
            return False
        return any([helper(i, cur + label(i)) for i in branches(t)]) 
...
def check_par():
    ...
    r2 = interval(2, 10)  # Replace this line!
    return r1, r2
