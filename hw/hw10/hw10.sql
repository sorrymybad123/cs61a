...
CREATE TABLE by_parent_height AS
  SELECT parents.child from dogs, parents where dogs.name = parents.parent order by height desc;  
...
CREATE TABLE sentences AS
  SELECT "The two siblings, " || a ||  " plus " || b || " have the same size: " 
         || size from siblings, size_of_dogs where name = a and 
	   (select size from size_of_dogs where name = a) = (select size from size_of_dogs 
		 where name = b);
