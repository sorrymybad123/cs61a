...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        if operator == 'and':  # and expressions
            ...
            return calc_apply(calc_eval(operator) ,operands)  # UPDATE THIS FOR Q3
    elif exp in OPERATORS:   # Looking up procedures
        return OPERATORS[exp]
    elif isinstance(exp, int) or isinstance(exp, bool):   # Numbers and booleans
        return exp
    elif exp in bindings.keys():  # CHANGE THIS CONDITION FOR Q5
...
def eval_and(operands):
    ...
    if operands.rest is nil:
        if isinstance(operands.first, Pair):
...
def eval_define(expr):
    ...
    # BEGIN SOLUTION Q5
    x = expr.first
    bindings[x] = calc_eval(expr.rest.first)
    return x
