...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            already_there = place.ant
            new_ant = self
            
            if already_there.is_container == True and already_there.can_contain(new_ant):
                already_there.store_ant(new_ant)
            elif new_ant.is_container == True and  new_ant.can_contain(already_there):
                new_ant.store_ant(already_there)
                place.ant = new_ant
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        transition = 0
        if now_place.bees:
            return random_bee(now_place.bees)  # REPLACE THIS LINE
        while(True):
            last_place = now_place
            transition += 1
            if now_place.entrance:
                now_place = now_place.entrance

            if  now_place.is_hive == False:
                if now_place.bees:
                    if self.lower_bound <= transition and transition <= self.upper_bound:
                        return random_bee(now_place.bees)  # REPLACE THIS LINE
            else:
                return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        target = ant_place.bees[:]
        super().reduce_health(amount)
        if self.health <= 0:
            for bee in target:
                Ant.reduce_health(bee, self.damage + amount)
        else: 
            for bee in target:
                Ant.reduce_health(bee, amount) 
...
class HungryAnt(Ant):
    ...
    def chew_bee(self, bee):
        Ant.reduce_health(bee, bee.health) 
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        for bee in bees:
            Ant.reduce_health(bee, self.damage)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while(behind != None):
            ...
            behind = now.exit 
            if behind:
                behind_ant = behind.ant
                if behind_ant:
                    behind_ant.double()
                    if behind_ant.is_container ==  True:
                        if behind_ant.ant_contained != None:
                            Ant.double(behind_ant.ant_contained)
        # END Problem 12
    ...
    def remove_from(self, place):
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        assert True, "{0} cannot died".format(self)
        # END Problem 12
