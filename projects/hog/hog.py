...
def tail_points(opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    assert type(opponent_score) == int,'oppent_score must be an integer'
    assert opponent_score >= 0, 'Must be positive.'
    i = 2
    a = 0
    b = 0
    while (i > 0):
...
def play(strategy0, strategy1, update,
    ...
    strategy1: The stra:tegy for player1.
    ...
    while (score0 < goal and score1 < goal):
        if who == 0:
            play0 = strategy0(score0, score1)
            score0 = update(play0, score0, score1, dice)
            who = 1
        elif who == 1:   
            play1 = strategy1(score1, score0)
            score1 = update(play1, score1, score0, dice)
            who = 0
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for i in range(1,11):
        test_dice = make_averaged(roll_dice, total_samples)
        average_score = test_dice(i, dice)
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    current_score = tail_points(opponent_score) + score
    if perfect_square(current_score):
        return 0 
    return tail_strategy(score, opponent_score, threshold, num_rolls)
    # END PROBLEM 11


def final_strategy(score, opponent_score):
    """当到达goal远远还不够时，可以采用square_strategy，当分数只差3分以内时，如果tail_point > 3则使用0点，如果不够时投2下.
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    goal_point = 100
    #当差3分以内就能到达目标分数时
    if score + 3 >= goal_point :
        #如果使用这个策略大于3时取0
        if tail_points(opponent_score) >= 3:
            return 0
        else:
            return 2
    return square_strategy(score, opponent_score)    
    # END PROBLEM 12
