...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        procedure = do_lambda_form(lambda_expression, env) 
...
def do_and_form(expressions, env):
    ...
    if is_scheme_false(val):
        return val 
    elif expressions.rest is nil and is_scheme_true(val):
        return val 
    else:
...
def do_or_form(expressions, env):
    ...
    if is_scheme_true(val):
        return val
    elif is_scheme_false(val) and expressions.rest is nil:
        return val 
    else:
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            if clause.rest is nil:
                return test 
            value = clause.rest.first
            if clause.rest.rest is not nil:
                return eval_all(clause.rest, env)
            else:
                return scheme_eval(value, env)
