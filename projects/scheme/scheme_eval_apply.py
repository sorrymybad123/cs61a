...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        args = rest.map(lambda s: scheme_eval(s, env))
        return scheme_apply(procedure, args, env) 
        # END PROBLEM 3

def convert_list(pair):
    if pair is nil:
        return []
    else:
        return [pair.first] + convert_list(pair.rest)
...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        return None
    new = scheme_eval(expressions.first, env)  # replace this with lines of your own code
